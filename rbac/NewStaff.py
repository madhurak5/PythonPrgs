...
#             # c = conn.cursor()
#             delUser = int(0)
#             recE = c.execute("select username from Staff where username = ?",(values['username'],))
#             recExist = recE.fetchall()
#             # print(recExist)
#             Role = ''
#             if recExist == '' or recExist == []:
#                 print("User can be created")
#                 # c.execute("alter table credForm1 add column Email")
#                 # c = conn.cursor()
#
#                 c.execute("insert into  Staff (username, Fname, Lname, Pwd, CPwd, DoB, Gender, Qualification, Designation, Experience, Department, Place, Email, Role, AARequest, DelUserStatus) "
#                           "values (?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?)",(values['username'], values['Fname'], values['Lname'], values['Pwd'], values['CPwd'], values['DoB'], gen, values['Qualification'], values['Designation'],values['Experience'],values['Department'],values['Place'],values['Email'],Role, reqAA, delUser,))
